...
class Relationship(BaseModel):
    ...
    source_field_schema: str = Field(...,
                                     description="The source field of the relationship.")
    target_field_schema: str = Field(...,
                                     description="The target field of the relationship.")
    source_field_table: str = Field(...,
                                    description="The source table of the relationship.")
    target_field_table: str = Field(...,
                                    description="The target table of the relationship.")
    source_field_column: str = Field(...,
                                     description="The source column of the relationship.")
    target_field_column: str = Field(...,
                                     description="The target column of the relationship.")
    target_field_lookup_column: Optional[str] = Field(
        default="", description="The target column of the relationship.")
...
def generate_date_templates(dep_graph: DepGraph, output_path: Path):
    ...
    for table_node in dep_graph.get_all_tables():
        ...
        template_file = output_path / \
            f"{table_node.schema_name}_{table_node.table_name}.csv"
        ...
        print(
            f"Generated data template for {table_node.full_table_name} at {template_file}")
   ...
   # First run through each of the table nodes in the dep graph and construct the relationships
    ...
    for table_node in dep_graph.get_all_tables():
        for parent_relationship in table_node.parent_relationships:
            ...
            relationships[parent_relationship.constraint_name] = Relationship(
    ...
    with open(output_path, "w") as f:
        ...
        print(f"Finished writing relationships at {output_path}")