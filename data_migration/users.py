...
class CognitoConfig(BaseModel):
    ...
    iam_access_key: str = Field(..., description="The client ID for the Cognito application.")
    iam_access_secret: str = Field(..., description="The client secret for the Cognito application.")
...
def create_cognito_users(users: List[UserModel], cognito_config: CognitoConfig):
    client = boto3.client(
        ...
        endpoint_url=f"https://cognito-idp.{cognito_config.region}.amazonaws.com",
        aws_access_key_id=cognito_config.iam_access_key,
        aws_secret_access_key=cognito_config.iam_access_secret,
    )
